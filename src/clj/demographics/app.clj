...
(defn get-meeting-info
  ...
  [id]
  (prn id)
  (resp/response @meeting-attendees))

(defn add-meeting-participant
  "Adds a participant to a meeting"
  [meeting-id primary-attr-val secondary-attr-val]
  (resp/response (register-participant! (keyword primary-attr-val) (keyword secondary-attr-val))))
...
(defn create-new-meeting [base-uri]
  (let [new-id (gen-chars 6)
    ...
    (-> (resp/response {:message "created" :_links {:self {:href  new-uri :type "application/json"}} :meeting-id new-id})
        (resp/header "Location" new-uri)
        (resp/status 202))))

(defroutes api-routes
  (GET ["/meeting/:id" :id re-meeting-id] [id] (get-meeting-info id))
